...
if ( ! function_exists( 'plamen_core_register_breadcrumbs_title_layout' ) ) {
	function plamen_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'PlamenCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'plamen_core_filter_register_title_layouts', 'plamen_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'plamen_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function plamen_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'plamen-core' );
		
		return $layouts;
	}
	
	add_filter( 'plamen_core_filter_title_layout_options', 'plamen_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'plamen_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function plamen_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();
		
		// Breadcrumbs label
		$labels = apply_filters( 'plamen_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'plamen-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'plamen-core' ),
			'author'      => esc_html__( 'Posted by %s', 'plamen-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'plamen-core' ),
			'404'         => esc_html__( '404 - Page not found', 'plamen-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'plamen-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'plamen_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'plamen_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'plamen_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}