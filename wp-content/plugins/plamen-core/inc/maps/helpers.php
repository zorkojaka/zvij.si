...
if ( ! function_exists( 'plamen_core_set_multiple_map_variables' ) ) {
	/**
	 ...
	 */
	function plamen_core_set_multiple_map_variables( $query = array(), $return = false ) {
		$map_variables = array();
		...
		if ( is_array( $query ) && count( $query ) ) {
			...
			if (!empty($items)) {
				foreach ($items as $id => $title) {
					$map_variables['addresses'][] = plamen_core_generate_map_params( $id, $query['post_type'] );
				}
			}
		}
		...
		wp_localize_script( 'plamen-core-google-map', 'qodefMultipleMapVars', array(
			'multiple' => $map_variables
		) );
	}
}

if ( ! function_exists( 'plamen_core_get_multiple_map' ) ) {
	/**
	 ...
	 * @return string
	 */
	function plamen_core_get_multiple_map( $query = array() ) {
		plamen_core_set_multiple_map_variables( $query );

		$google_maps_api_key = plamen_core_get_option_value( 'admin', 'qodef_maps_api_key' );
		if ( ! empty( $google_maps_api_key ) ) {
			$html = '<div id="qodef-multiple-map-holder"></div>';
		} else {
			$html = '<p id="qodef-multiple-map-notice">' . esc_html__( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Plamen Options', 'plamen-core' ) . '</p>';
		}

		do_action( 'plamen_core_action_after_multiple_map' );
...
if ( ! function_exists( 'plamen_core_marker_info_template' ) ) {
	/**
	 * Template with placeholders for marker info window
	 *
	 * uses underscore templates
	 */
	function plamen_core_marker_info_template() {
	...
	add_action( 'plamen_core_action_after_multiple_map', 'plamen_core_marker_info_template' );
}

if ( ! function_exists( 'plamen_core_marker_template' ) ) {
	/**
	 * Template with placeholders for marker
	 */
	function plamen_core_marker_template() {
	...
	add_action( 'plamen_core_action_after_multiple_map', 'plamen_core_marker_template' );
}
...
if ( ! function_exists( 'plamen_core_generate_map_params' ) ) {
	function plamen_core_generate_map_params( $item_id, $post_type ) {
		$map_params = array();
		...
		//get address params
		$address_array = apply_filters( 'plamen_core_filter_address_params', array(), $item_id, $post_type );
