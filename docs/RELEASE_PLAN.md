...
| 4 | zvijace@zvij.si + SPF/DKIM/DMARC | Jaka (cPanel/DNS) | 🔧 skoraj urejeno (11. 7.): nabiralnik zvijace@zvij.si obstaja (SMTP auth), SPF in DKIM (selector `default`) sta že v DNS pri Neoservu, mail-tester **10/10**. Manjka samo DMARC TXT: ime `_dmarc`, vrednost `v=DMARC1; p=none; rua=mailto:zvijace@zvij.si; fo=1` (Neoserv cPanel → Zone Editor); po 2–4 tednih čistih poročil dvigni na `p=quarantine` |
