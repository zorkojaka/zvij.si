...
if ( ! function_exists( 'plamen_core_add_banner_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function plamen_core_add_banner_shortcode( $shortcodes ) {
		$shortcodes[] = 'PlamenCoreBannerShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'plamen_core_filter_register_shortcodes', 'plamen_core_add_banner_shortcode' );
}

if ( class_exists( 'PlamenCoreShortcode' ) ) {
	class PlamenCoreBannerShortcode extends PlamenCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'plamen_core_filter_banner_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'plamen_core_filter_banner_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( PLAMEN_CORE_SHORTCODES_URL_PATH . '/banner' );
			$this->set_base( 'plamen_core_banner' );
			$this->set_name( esc_html__( 'Banner', 'plamen-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds banner element', 'plamen-core' ) );
			...
			$this->set_option( array(
				...
				'options'       => plamen_core_get_select_type_options_pool( 'title_tag', true, '', array('span') ),
				'default_value' => 'span',
				'group'         => esc_html__( 'Content', 'plamen-core' )
			) );
			$this->set_option( array(
            ...
            $this->set_option( array(
                'field_type'    => 'select',
                'name'          => 'title_outlined',
                'title'         => esc_html__( 'Enable Outlined Title', 'plamen-core' ),
                'description'   => esc_html__( 'Title is transparent, outlined in white', 'plamen-core' ),
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'subtitle_tag',
				'title'         => esc_html__( 'Subtitle Tag', 'plamen-core' ),
			...
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'subtitle_color',
				'title'      => esc_html__( 'Subtitle Color', 'plamen-core' ),
				'group'      => esc_html__( 'Content', 'plamen-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'subtitle_margin_top',
				'title'      => esc_html__( 'Subtitle Margin Top', 'plamen-core' ),
				'group'      => esc_html__( 'Content', 'plamen-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
			...
			$this->set_option( array(
				'field_type'    => 'select',
				'name'          => 'text_tag',
				'title'         => esc_html__( 'Text Tag', 'plamen-core' ),
				'options'       => plamen_core_get_select_type_options_pool( 'title_tag' ),
				'default_value' => 'p',
				'group'         => esc_html__( 'Content', 'plamen-core' )
			) );
		...
		public function render( $options, $content = null ) {
			...
			return plamen_core_get_template_part( 'shortcodes/banner', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes[] = ! empty ( $atts['title_outlined'] ) && $atts['title_outlined'] === 'yes' ? 'qodef-banner-outlined-title' : '';
		...
		private function generate_button_params( $atts ) {
			...
			$params['button_layout'] = ! empty( $atts['button_layout'] ) ? $atts['button_layout'] : 'textual';
