...
6. ~~Analitika~~ **Urejeno (11. 7.): Plausible CE (samohostan)** — stack v `/home/jaka/apps/plausible` (port 8100), instanca `https://analitika.zvij.si`, admin račun zorkojaka@gmail.com (začetno geslo: `/home/jaka/apps/plausible/.admin-initial-password`, registracije zaklenjene). Strani dev.inteligent.si + zvij.si, cilji view_item/add_to_cart/begin_checkout/purchase (purchase z EUR revenue). Tema pošilja dogodke (`assets/analytics.js`), brez piškotkov → brez privolitve. **Čaka:** Jaka doda DNS A zapis `analitika` → `178.104.24.47` v zvij.si cono, nato agent izda certifikat in E2E test.
