...
if ( ! function_exists( 'plamen_core_add_separator_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function plamen_core_add_separator_shortcode( $shortcodes ) {
		$shortcodes[] = 'PlamenCoreSeparatorShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'plamen_core_filter_register_shortcodes', 'plamen_core_add_separator_shortcode', 9 );
}

if ( class_exists( 'PlamenCoreShortcode' ) ) {
	class PlamenCoreSeparatorShortcode extends PlamenCoreShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( PLAMEN_CORE_SHORTCODES_URL_PATH . '/separator' );
			$this->set_base( 'plamen_core_separator' );
			$this->set_name( esc_html__( 'Separator', 'plamen-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays separator with provided parameters', 'plamen-core' ) );
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'position',
				'title'      => esc_html__( 'Position', 'plamen-core' ),
				'options'    => array(
			...
			$this->set_option( array(
				'field_type' => 'select',
				'name'       => 'border_style',
				'title'      => esc_html__( 'Border Style', 'plamen-core' ),
				'options'    => plamen_core_get_select_type_options_pool( 'border_style' ),
				'group'      => esc_html__( 'Style', 'plamen-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'width',
				'title'      => esc_html__( 'Width (px or %)', 'plamen-core' ),
				'group'      => esc_html__( 'Style', 'plamen-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'thickness',
				'title'      => esc_html__( 'Thickness (px)', 'plamen-core' ),
				'group'      => esc_html__( 'Style', 'plamen-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_top',
				'title'      => esc_html__( 'Margin Top (px or %)', 'plamen-core' ),
				'group'      => esc_html__( 'Style', 'plamen-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'margin_bottom',
				'title'      => esc_html__( 'Margin Bottom (px or %)', 'plamen-core' ),
				'group'      => esc_html__( 'Style', 'plamen-core' )
			) );
		}
		...
		public function render( $options, $content = null ) {
			...
			return plamen_core_get_template_part( 'shortcodes/separator', 'templates/separator', '', $atts );
		}
