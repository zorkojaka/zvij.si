...
if ( ! function_exists( 'plamen_core_add_image_gallery_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function plamen_core_add_image_gallery_shortcode( $shortcodes ) {
		$shortcodes[] = 'PlamenCoreImageGalleryShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'plamen_core_filter_register_shortcodes', 'plamen_core_add_image_gallery_shortcode' );
}

if ( class_exists( 'PlamenCoreListShortcode' ) ) {
	class PlamenCoreImageGalleryShortcode extends PlamenCoreListShortcode {
		
		public function map_shortcode() {
			$this->set_shortcode_path( PLAMEN_CORE_SHORTCODES_URL_PATH . '/image-gallery' );
			$this->set_base( 'plamen_core_image_gallery' );
			$this->set_name( esc_html__( 'Image Gallery', 'plamen-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds image gallery element', 'plamen-core' ) );
			...
			$this->set_option( array(
				...
				'name'       => 'images',
				'multiple'   => 'yes',
				'title'      => esc_html__( 'Images', 'plamen-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'text',
				'name'       => 'image_size',
			...
			$this->set_option( array(
				...
				'options'    => array(
					...
					'open-popup'  => esc_html__( 'Open Popup', 'plamen-core' ),
			...
			$this->map_list_options( array(
				...
				'group'            => esc_html__( 'Gallery Settings', 'plamen-core' )
			) );
		}

		public static function call_shortcode( $params ) {
			$html = qode_framework_call_shortcode( 'plamen_core_image_gallery', $params );
			$html = str_replace( "\n", '', $html );
		...
		public function render( $options, $content = null ) {
			...
			return plamen_core_get_template_part( 'shortcodes/image-gallery', 'templates/image-gallery', $atts['behavior'], $atts );
		}
