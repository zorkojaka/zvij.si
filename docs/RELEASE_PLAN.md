...
> **Posodobitev 4. 9. 2026:** **tečaj kristalov 10:1 → 100:1** in **količinska lestvica**. En kristal je odslej en cent; vse vrednosti pomnožene z 10 (DUBI 42 130, DUBI 420 750, vršički 80–450), nagrada za Zvij kodo 300. Ledger je bil prazen, zato ni bilo treba preračunavati stanj članov. Ziggi izdelki zdaj dajejo kristale (10 / 12 / 15 / 20 na kos). Ob prehodu odkrita in odpravljena podvojitev: meta `_zvij_dobroimetje_note` je hranila isti podatek kot `_zvij_kristali`, a z zamrznjeno številko — sistem je pripisoval 130, na strani je pisalo 13; napis se odslej izpelje prek `zvij_credit_public_note()`. **Količinska lestvica** (zvij-core 0.12.0): 3 kosi −10 %, 10 kosov −15 %, cela škatla −20 % (prag = pakiranje dobavitelja, 26/22/16/14). Izvedeno kot količinski prag, ne kot ločen izdelek »škatla«, da zaloga ostane ena sama. Nakupni tok dobil besedišče **grindanje → zvijanje → prižgi**: metafora nosi košarico, na blagajni pa je samo korakovnik in pripis pod gumbom — napisi, ki vodijo plačilo, ostajajo nedvoumni.
>
> **Posodobitev 1. 9. 2026:** **potrjene zaloge in grinderji.** DUBI 420 = 5 paketov (račun je bil 15, 10 porabljenih), DUBI 42 = 50. Zlat grinder (POLI-110, 34,90 €, 1 kos) in mali srebrn grinder (GRI-M-03, 8,90 €, 3 kosi) objavljena z natančnimi imeni, opisi in očiščenimi slugi (`gold-grinder-placeholder` → `zlat-grinder-52-mm`, s 301 s starega). Clipper Silver ostaja osnutek — nabave ni. **Vžigalniki z druge proforme niso dvojno plačani** — Knistermann je znesek vrnil od plačila prvega računa; opomba v ceniku popravljena. Vodena zaloga zdaj na 17 izdelkih, vrednost **1.864,84 €**. Dodan `scripts/sync-dev-to-prod.sh` — izvedbeni del Faze 2 runbooka kot en ponovljiv ukaz. **Odprto:** zaloge za 14 izdelkov, 13 izdelkov brez prave fotografije, marža na zlatem grinderju je 35 % (pod ciljnim minimumom 40 %), Revolut ključ, pravni pregled, DNS.
>
